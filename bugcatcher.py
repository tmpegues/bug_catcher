...
class BugCatcher:
    ...
    async def ambushing_pick(self, bridge_end_position: Point) -> (bool):
        ...
        ambush_pose.position.z = bridge_end_position.z + self.GRIPPER_OFFSET_Z  # Hover slightly above the bridge height
        ambush_pose.orientation = Quaternion(x=1.0, y=0.0, z=0.0, w=0.0)  # TODO: Update with correct orientation
        ...
        if t_bug_arrival < t_robot_travel:
            self.node.get_logger().info(
                'Ambush aborted: Robot cannot reach ambush point '
                'before bug arrives.')
            return False
